...
def Generator(n_samples, noise, idx):
    ...
    #output_distribution = tf.contrib.distributions.Laplace(name=prefix + '.Noise', loc=output_mean, scale=output_scale)
...
def Encoder(output, idx, bijectorCount=0, invert=False, name="Encoder"):
    prefix = name + str(idx)
...
GeneratorFun = lambda data, noise, idx: Generator(data, noise, idx)
EncoderFun = lambda data, idx: Encoder(data, idx, bijectorCount=0, invert=False) 
...
fake_enc2_gen, fake_enc2_gen_mean, fake_enc2_gen_scale, fake_enc2_gen_eval_fun = GeneratorFun(BATCH_SIZE, fake_enc2, 0)
...
model_gen, model_gen_mean, model_gen_scale, model_gen_eval_fun = GeneratorFun(BATCH_SIZE, model_noise, enc3_idx)
fake_enc3, fake_enc3_mean, fake_enc3_scale, fake_enc3_eval_fun = EncoderFun(fake_data, enc3_idx)
fake_enc3_gen, fake_enc3_gen_mean, fake_enc3_gen_scale, fake_enc3_gen_eval_fun = GeneratorFun(BATCH_SIZE, fake_enc3, enc3_idx)
...
if MODE == 'wgan':
    ...
    entropy3_adjustmentx = tf.reduce_mean(fake_enc3_log_ratio1)
    ...
    enc_cost = entropy3 + entropy2 - entropy1
    ...
    enc_log_ratio_cost_regularization = 0
    enc_log_ratio_cost = enc1_log_ratio_cost + enc2_log_ratio_cost + enc3_log_ratio_cost + enc_log_ratio_cost_regularization
    ...
    model_train_op = tf.train.AdamOptimizer(
        ...
        beta1=0.5,
        beta2=0.9
    ).minimize(model_cost, var_list=model_params)
...
fixed_noise_samples_enc2_gen = GeneratorFun(128,EncoderFun(fixed_noise_samples, enc2_idx)[0], 0)[1]
fixed_noise_samples_enc3_gen = GeneratorFun(128,EncoderFun(fixed_noise_samples, enc3_idx)[0], 3)[1]
    ...
    for iteration in xrange(ITERS):
        ...
        for i in xrange(disc_iters):
            ...
            if i == 0:
                ...
                _entropy1, _entropy1_adjustment, _entropy2, _entropy2_adjustment, _entropy3, _entropy3_adjustmentx, _entropy3_adjustment, _fake_gen_scale_sum, _fake_enc1_scale_sum, _fake_enc2_scale_sum, _fake_enc2_gen_scale_sum, _gen_cost, _model_cost, _enc_cost, _enc1_log_ratio_cost, _enc2_log_ratio_cost, _enc3_log_ratio_cost, _disc_cost, _, _, _, _, _global_step = session.run([entropy1, entropy1_adjustment, entropy2, entropy2_adjustment, entropy3, entropy3_adjustmentx, entropy3_adjustment, fake_gen_scale_sum, fake_enc1_scale_sum, fake_enc2_scale_sum, fake_enc2_gen_scale_sum, gen_cost, model_cost, enc_cost, enc1_log_ratio_cost, enc2_log_ratio_cost, enc3_log_ratio_cost, disc_cost, gen_train_op, enc_train_op, enc_log_ratio_train_op, disc_train_op, global_step], feed_dict={real_data: _data})
        ...
        print "gen_cost=%s model_cost=%s enc_cost=%s enc_log_ratio_cost=%s/%s/%s disc_cost=%s entropy1=%s (%s) entropy2=%s (%s) entropy3=%s (%s/%s) time=%s" % (_gen_cost, _model_cost, _enc_cost, _enc1_log_ratio_cost, _enc2_log_ratio_cost, _enc3_log_ratio_cost, _disc_cost, _entropy1,  _entropy1_adjustment, _entropy2, _entropy2_adjustment, _entropy3, _entropy3_adjustmentx, _entropy3_adjustment, time.time() - start_time)
